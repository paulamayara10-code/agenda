...
def create_user(name: str, role: str = "Usuário", department: str = "") -> None:
    """
    Cria ou atualiza usuário sem rebaixar perfis administrativos existentes.
    A importação das rotinas pode reencontrar Paula como responsável, mas
    isso não deve substituir Administradora por Usuário.
    """
    ...
    execute(
        ...
        ON CONFLICT(name) DO UPDATE SET
            role=CASE
                WHEN LOWER(users.role) IN ('administrador', 'administradora', 'admin')
                    THEN users.role
                WHEN LOWER(excluded.role) IN ('administrador', 'administradora', 'admin')
                    THEN excluded.role
                WHEN LOWER(users.role) IN ('coordenador', 'coordenadora')
                     AND LOWER(excluded.role) = 'usuário'
                    THEN users.role
                ELSE excluded.role
            END,
            department=CASE
                WHEN TRIM(COALESCE(excluded.department,'')) <> ''
                    THEN excluded.department
