...
section[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0f172a 0%, #111827 70%, #1e1b4b 100%);
}
...
.hero {
    ...
    border-radius: 30px;
    padding: 28px 32px;
    box-shadow: 0 22px 65px rgba(49,46,129,0.25);
    margin-bottom: 24px;
}
...
def header(title, subtitle):
    ...
    with col2:
        st.markdown(
            f"<div style='text-align:right;font-weight:800;color:#0f172a;'>📅 {datetime.now().strftime('%d/%m/%Y • %H:%M')}</div>",
            unsafe_allow_html=True
        )
    ...
    with st.container(border=True):
        ...
        with st.expander("💬 Observações / justificativa", expanded=False):
            observacao = st.text_area(
                ...
                placeholder="Ex.: iniciado, aguardando documento, faltou retorno do banco...",
                key=f"obs_{prefix}_{idt}"
            )
...
def coordenacao(user):
    ...
    for resp in sorted([x for x in tarefas["responsavel"].dropna().unique() if txt(x)]):
        base = tarefas[tarefas["responsavel"].astype(str).str.contains(resp, case=False, na=False)]
    ...
    if hist.empty:
        ...
        for _, row in hist.head(15).iterrows():
            st.markdown(
                f"**{row.get('hora','')}** — {row.get('usuario','')} registrou **{row.get('status','')}** em “{row.get('tarefa','')}”"
            )
...
def departamento_page(user, departamento):
    ...
    if busca:
        b = busca.lower()
        base = base[
            base["tarefa"].astype(str).str.lower().str.contains(b, na=False)
            | base["descricao"].astype(str).str.lower().str.contains(b, na=False)
        ]
...
def cadastro(user):
    ...
    if salvar:
        ...
        dados = {
            ...
            "prazo_limite": prazo_limite.strftime("%H:%M:%S"),
...
def editar(user):
    ...
    if salvar:
        dados = {
            ...
            "responsavel": responsavel,
            "periodicidade": periodicidade,
            "obrigatoria": txt(row.get("obrigatoria")) or "Não",
            "prioridade": prioridade,
            "dependencia": txt(row.get("dependencia")),
            "prazo_limite": txt(row.get("prazo_limite")),
            "data_inicio": txt(row.get("data_inicio")),
            "status": status,
            "observacao": observacao,
        }
...
def calendario(user):
    ...
    previstas = tarefas[
        ...
        & (~tarefas.apply(lambda r: is_concluida(r, ref), axis=1))
    ]
...
def main():
    ...
    if page == "Dashboard":
        ...
        editar(user)
    elif page == "Pendências com observação":
        pendencias_observacao(user)
